...
export async function POST(request: NextRequest) {
  try {
    // --- MANUAL MODE SWITCH ---
    // User requested to disable automated logic due to missing production keys.
    // We return 200 OK immediately to prevent errors if Square retries.
    console.log("Square Webhook received but disabled via config (Manual Mode).");
